...
function DeleteModal({focusedTeam, isOpen, hide}) {
return(
  <div tabIndex='-1' className={'overflow-y-auto fixed top-1/4 left-1/2 -translate-x-1/2 z-50 h-modal md:h-full transition ease-in-out'+ (isOpen ? ' visible opacity-100' : ' invisible opacity-0')}>
    <div className='relative p-4 w-full max-w-md h-full md:h-auto'>
        <div className='relative bg-white rounded-lg shadow dark:bg-gray-700'>
            ...
            <div className='p-6 text-center'>
                ...
                <h3 className='mb-5 text-lg font-normal text-gray-500 dark:text-gray-400'>Are you sure you want to delete {focusedTeam.name}?</h3>
                <button type='button' className='text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2' onClick={() =>{api.deleteTeam(focusedTeam?.tla); hide()}}>
                    Yes, Im sure
                </button>
                <button type='button' className='text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600' onClick={hide}>No, cancel</button>
            </div>
        </div>
    </div>
</div>
